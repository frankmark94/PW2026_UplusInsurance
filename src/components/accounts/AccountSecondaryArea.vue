<template>
  <div class="col col-2">
    <div
      ...
      "
    >
      <section v-if="app.industry === 'insurance'" class="offer-card-col">
        <img
          class="offer-img"
          :src="'./img/UberDriverstock.jpg'"
          :alt="$t('message.ai_assistant_title')"
        />
        <div class="content">
          <h3>{{ $t('message.ai_assistant_title') }}</h3>
          <p>{{ $t('message.ai_assistant_msg') }}</p>
          <a href="./rideshare.html">{{ $t('message.ai_assistant_cta') }}</a>
        </div>
      </section>
      <section
